...
module.exports = class ApiClient {
  ...
  getTalks() {
    return this.firestore.collection('talks').get().then(snap => {
  ...
  getNews() {
    return this.firestore.collection('news').get().then(snap => {
      return this.parseNews(snap.docs.map(doc => doc.data()));
    })
    .catch(error => {
  ...
  parseTalks(talks) {
    ...
    for (talk of talks) {
      const talkDate = talk.day;
      console.log(talkDate);
      if (talkDate == 'Monday 16th') {
        sections[0].data.push(talk);
      } else {
        sections[1].data.push(talk);
      }
    }
    console.log(`sections: ${JSON.stringify(talks, null, 2)}`);
